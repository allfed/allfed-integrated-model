...
df_pop_head_country["pig_per_head"] = df_pop_head_country["medium_animals"] / df_pop_head_country["population"]
df_pop_head_country["beef_per_head"] = df_pop_head_country["large_animals"] / df_pop_head_country["population"]
df_pop_head_country["dairy_per_head"] = df_pop_head_country["dairy_cows"] / df_pop_head_country["population"]

# slaughter to head ratio, small_animal_slaughter,medium_animal_slaughter,large_animal_slaughter
df_pop_head_country["poultry_slaughter_to_head"] = df_pop_head_country["small_animals"] / df_slaughter_country["small_animal_slaughter"]
df_pop_head_country["pig_slaughter_to_head"] = df_pop_head_country["medium_animals"] / df_slaughter_country["medium_animal_slaughter"]
df_pop_head_country["beef_slaughter_to_head"] = df_pop_head_country["large_animals"] / df_slaughter_country["large_animal_slaughter"]


# plotly to plot head per pop, sorted by beef per head
fig = px.bar(df_pop_head_country.sort_values(by="country_x"), x="country_x", y=["poultry_per_head", "pig_per_head", "beef_per_head", "dairy_per_head"], title="Species Per Head")
fig.update_layout(title_text="Species Per Head")
fig.show()

# plotly to plot slaughter to head ratio, sorted by beef per head
fig2 = px.bar(df_pop_head_country.sort_values(by="country_x"), x="country_x", y=["poultry_slaughter_to_head", "pig_slaughter_to_head", "beef_slaughter_to_head"], title="Slaughter to Head Ratio")
