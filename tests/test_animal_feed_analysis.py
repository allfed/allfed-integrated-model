...
import pandas as pd
from scripts.animal_population_analysis.animal_feed_analysis import (
    add_alpha_codes_from_ISO,
)
from scripts.animal_population_analysis.animal_feed_analysis import (
    add_alpha_codes_fuzzy,
)
from scripts.animal_population_analysis.animal_feed_analysis import (
    bulk_correlation_analysis,
)
...
Objective:
The objective of the function is to add a column of alpha3 codes to a dataframe with country names in a specified
column. The function uses fuzzy logic to match countries and retrieves the alpha3 codes from the pycountry library.
...
- If a match is not found, set the alpha3 code to "NA" and print a message indicating the input country could not be
  matched
...
- If a country in the input column could not be matched, a message will be printed indicating the country could not be
  matched and the corresponding alpha3 code will be set to "NA"

Additional aspects:
- The function uses the pycountry library to retrieve alpha3 codes for countries
- Fuzzy logic is used to match countries in case the input country name is not an exact match with a country in the
  pycountry library
- The function drops the original country name column from the dataframe and returns the modified dataframe with the
  new alpha3 code column
"""
...
Objective:
The objective of the 'add_alpha_codes_fuzzy' function is to add a column of alpha3 codes to a dataframe with country
names in a specified column. The function uses fuzzy logic to match countries and handle cases where the input country
...
2. Iterate through each input country name and use fuzzy logic to search for a matching country in the pycountry
   library.
...
- Fuzzy logic is used to handle cases where the input country name is not an exact match to a country name in the
  library.
...
Objective:
The objective of the 'bulk_correlation_analysis' function is to calculate the correlation between a target column and
all other columns in a given DataFrame, and to visualize the correlation using a scatter plot.

Inputs:
- df: a pandas DataFrame containing the data to be analyzed
- target_column: a string representing the name of the target column for which the correlation with other columns will
  be calculated
