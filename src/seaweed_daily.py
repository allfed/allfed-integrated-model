...
built_area=np.linspace(INITIAL_AREA,(NDAYS-1)*NEW_AREA_PER_DAY+INITIAL_AREA,250)
...
for d in range(0,NDAYS):
	...
	if(d>=150):
		maximizer_string="Seaweed_Food_Produced_"+str(d)+"_Objective_Constraint"
...
WASTE=1/(1-13/100)
DAYS_IN_MONTH=30.4
STANDARD_KCALS_DAILY=2100
KCALS_MONTHLY=STANDARD_KCALS_DAILY*DAYS_IN_MONTH
USED_KCALS_MONTHLY=KCALS_MONTHLY*WASTE/1e9*1e6 #in billions of kcals per million persons
...
max_world_calories_wet_tonnage_month=WORLD_POP*USED_KCALS_MONTHLY/SEAWEED_KCALS
...
plt.title('Seaweed Wet, 1000s of tons, Optimize Food after day 150')
plt.yscale('log')
plt.show()
...
plt.plot(np.array(seaweed_food_produced_vals).cumsum())
plt.legend([
	'Max Density times Max Area',
	'7.9 billion people monthly calorie requirement',
	# 'Built Area times Max Density',
