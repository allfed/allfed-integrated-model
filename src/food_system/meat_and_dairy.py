...
class MeatAndDairy:
    ...
    def __init__(self,inputs_to_optimizer):
        ...
        self.LARGE_ANIMAL_FAT_PER_KG = .182
        self.LARGE_ANIMAL_PROTEIN_PER_KG = .257

        self.SMALL_ANIMAL_KCALS_PER_KG = 1525
        self.SMALL_ANIMAL_FAT_PER_KG = 0.076
        self.SMALL_ANIMAL_PROTEIN_PER_KG = .196
        ...
        self.MEDIUM_ANIMAL_FAT_PER_KG = .34
        self.MEDIUM_ANIMAL_PROTEIN_PER_KG = .11
        ...
        self.MILK_FAT = .032  # kg per kg
        self.MILK_PROTEIN = .033  # kg per kg
        ...
        self.TONS_BEEF_MONTHLY_BASELINE = \
            inputs_to_optimizer['TONS_BEEF_ANNUAL'] / 12  # tons a month meat
        ...
        self.INIT_LARGE_ANIMALS = inputs_to_optimizer["INIT_LARGE_ANIMALS_WITH_MILK_COWS"] - INITIAL_MILK_CATTLE


    def calculate_meat_nutrition(self):
        KG_TO_1000_TONS = self.KG_TO_1000_TONS

        KCALS_PER_SMALL_ANIMAL = self.SMALL_ANIMAL_KCALS_PER_KG *\
            self.KG_PER_SMALL_ANIMAL / 1e9
        FAT_PER_SMALL_ANIMAL = self.SMALL_ANIMAL_FAT_PER_KG * \
            self.KG_PER_SMALL_ANIMAL * KG_TO_1000_TONS
        PROTEIN_PER_SMALL_ANIMAL = self.SMALL_ANIMAL_PROTEIN_PER_KG * \
            self.KG_PER_SMALL_ANIMAL * KG_TO_1000_TONS

        KCALS_PER_MEDIUM_ANIMAL = self.MEDIUM_ANIMAL_KCALS_PER_KG * \
            self.KG_PER_MEDIUM_ANIMAL / 1e9
        FAT_PER_MEDIUM_ANIMAL = self.MEDIUM_ANIMAL_FAT_PER_KG * \
            self.KG_PER_MEDIUM_ANIMAL * KG_TO_1000_TONS
        PROTEIN_MEDIUM_ANIMAL = self.MEDIUM_ANIMAL_PROTEIN_PER_KG * \
            self.KG_PER_MEDIUM_ANIMAL * KG_TO_1000_TONS

        KCALS_PER_LARGE_ANIMAL = self.LARGE_ANIMAL_KCALS_PER_KG * \
            self.KG_PER_LARGE_ANIMAL / 1e9
        FAT_PER_LARGE_ANIMAL = self.LARGE_ANIMAL_FAT_PER_KG * \
            self.KG_PER_LARGE_ANIMAL * KG_TO_1000_TONS
        PROTEIN_PER_LARGE_ANIMAL = self.LARGE_ANIMAL_PROTEIN_PER_KG * \
            self.KG_PER_LARGE_ANIMAL * KG_TO_1000_TONS
        ...
        self.INITIAL_MEAT = INIT_MEAT_KCALS \
            * (1 - self.MEAT_WASTE / 100)

        if(self.INITIAL_MEAT > 0):
            self.MEAT_FRACTION_FAT = INIT_MEAT_FAT \
                / INIT_MEAT_KCALS
            self.MEAT_FRACTION_PROTEIN = INIT_MEAT_PROTEIN \
                / INIT_MEAT_KCALS
        else:
    ...
    def calculate_meat_and_dairy_from_excess(self,kcals_fed_to_animals):
        ...
        h_e_fed_dairy_limit_food_usage = self.h_e_fed_dairy_limit\
        ...
        CHICKEN_PORK_LIMIT_FOOD_USAGE = self.CHICKEN_AND_PORK_LIMIT\
    ...
    def get_meat_from_human_edible_feed(self):
        ...
        present_day_tons_per_month_chicken_pork = \
            self.CHICKEN_AND_PORK_LIMIT  # tons a month

        # does not consider waste
        ratio_maintained_cattle = (np.array(
            self.cattle_maintained) + np.array(self.cattle_h_e_maintained)) / present_day_tons_per_month_cattle
        ...
        self.ratio_not_maintained_cattle[
            self.ratio_not_maintained_cattle < 0
        ] = 0

        # does not consider waste
        self.ratio_maintained_chicken_pork = np.array(
            self.chicken_pork_maintained) \
            / present_day_tons_per_month_chicken_pork
        ...
        small_to_medium_ratio \
            ...
            / (self.INIT_MEDIUM_ANIMALS * self.KG_PER_MEDIUM_ANIMAL + self.INIT_SMALL_ANIMALS * self.KG_PER_SMALL_ANIMAL)

        # billions kcals monthly
        self.chicken_pork_kcals = np.array(self.chicken_pork_maintained) * 1e3\
        ...
        h_e_meat_kcals = \
            np.array(cattle_h_e_maintained_kcals + self.chicken_pork_kcals) \
            * (1 - self.MEAT_WASTE / 100)
        h_e_meat_fat = \
        ...
        return (self.chicken_pork_kcals,
                chicken_pork_fat,
    ...
    def calculate_meat_dairy_from_human_inedible_feed(self,inputs_to_optimizer):
        ...
        for m in range(0, self.NMONTHS):
            if(self.ADD_DAIRY):
                max_dairy = self.human_inedible_feed[m] \
                    / self.INEDIBLE_TO_DAIRY_CONVERSION
        ...
        self.h_e_fed_dairy_limit = DAIRY_LIMIT \
                                   - np.array(self.dairy_milk_produced)

    def get_dairy_produced(self):
        # billions kcals
        dairy_milk_kcals = np.array(self.dairy_milk_produced) * 1e3\
            * self.MILK_KCALS / 1e9 * (1 - self.DAIRY_WASTE / 100)

        # thousands tons
        dairy_milk_fat = np.array(self.dairy_milk_produced) / 1e3\
            * self.MILK_FAT * (1 - self.DAIRY_WASTE / 100)

        # thousands tons
        dairy_milk_protein = np.array(self.dairy_milk_produced) / 1e3\
            * self.MILK_PROTEIN * (1 - self.DAIRY_WASTE / 100)

        return (dairy_milk_kcals,
                dairy_milk_fat,
                dairy_milk_protein)
    ...
    def get_cattle_maintained(self):
        ...
        if(self.ADD_MEAT):
            ...
            # billions kcals
            cattle_maintained_kcals = np.array(self.cattle_maintained)\
                * 1000  \
                * self.LARGE_ANIMAL_KCALS_PER_KG \
                / 1e9\
                * (1 - self.MEAT_WASTE / 100)
            ...
            cattle_maintained_fat = cattle_maintained_kcals * 1e9 \
                * self.LARGE_ANIMAL_FAT_PER_KG / self.LARGE_ANIMAL_KCALS_PER_KG / 1e6
    ...
    def get_excess(self,inputs_to_optimizer,biofuels,feed):
        ...
        self.excess_kcals = inputs_to_optimizer["EXCESS_CALORIES"] \
            + biofuels.biofuels_kcals \
            + feed.feed_shutoff_kcals

        self.excess_fat_used = nonshutoff_excess_fat_used\
            + biofuels.biofuels_fat \
            + feed.feed_shutoff_fat

        self.excess_protein_used = nonshutoff_excess_protein_used\
    ...
    def get_culled_meat(self,inputs_to_optimizer,feed_shutoff_delay_months):
        ...
        if(inputs_to_optimizer['CULL_ANIMALS']):
            self.CULL_DURATION_MONTHS = \
                inputs_to_optimizer["CULL_DURATION_MONTHS"]
            if(self.CULL_DURATION_MONTHS != 0):
                meat_culled =\
                    [0] * feed_shutoff_delay_months\
                    + [self.INITIAL_MEAT / self.CULL_DURATION_MONTHS] * self.CULL_DURATION_MONTHS\
                    + [0] * (self.NMONTHS - self.CULL_DURATION_MONTHS - \
    ...
    def get_dairy_from_human_edible_feed(self,inputs_to_optimizer):
        ...
        if(self.ADD_DAIRY):
            ...
            h_e_milk_kcals = np.array(self.h_e_fed_dairy_produced) * 1e3\
                * self.MILK_KCALS / 1e9 * (1 - self.DAIRY_WASTE / 100)

            h_e_milk_fat = np.array(self.h_e_fed_dairy_produced) / 1e3\
                * self.MILK_FAT * (1 - self.DAIRY_WASTE / 100)

            h_e_milk_protein = np.array(self.h_e_fed_dairy_produced) / 1e3\
                * self.MILK_PROTEIN * (1 - self.DAIRY_WASTE / 100)
