...
def read_animal_population_data():
    ...
    df_animal_stock_info = pd.merge(
        df_head_country, df_slaughter_country, left_index=True, right_index=True
    )
...
def read_animal_nutrition_data():
    ...
    df_animal_nutrition = pd.read_csv(
        animal_nutrition_data_location, index_col="animal_type"
    )
...
def create_animal_objects(df_animal_stock_info, df_animal_nutrition):
    ...
    # Create obnjects for each animal type, using the class SpeciesCurrentState, and populate with data
    # from the dataframe
