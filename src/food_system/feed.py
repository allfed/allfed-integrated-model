...
class Feed:
    ...
    def __init__(self,inputs_to_optimizer):
        ...
        self.FEED_MONTHLY_USAGE_KCALS = self.FEED_KCALS / 12 * 4e6 / 1e9  # billions kcals
    ...
    def get_feed_usage(self,inputs_to_optimizer):
        ...
        self.feed_shutoff_delay_months = inputs_to_optimizer["DELAY"]["FEED_SHUTOFF_MONTHS"]
        self.feed_shutoff_kcals = np.array(
            [self.FEED_MONTHLY_USAGE_KCALS] * self.feed_shutoff_delay_months + [0] * (self.NMONTHS-self.feed_shutoff_delay_months))
        self.feed_shutoff_fat = np.array(
            [self.FEED_MONTHLY_USAGE_FAT] * self.feed_shutoff_delay_months + [0] * (self.NMONTHS-self.feed_shutoff_delay_months))
        self.feed_shutoff_protein = np.array(
            [self.FEED_MONTHLY_USAGE_PROTEIN] * self.feed_shutoff_delay_months + [0] * (self.NMONTHS-self.feed_shutoff_delay_months))
            
        self.kcals_fed_to_animals = inputs_to_optimizer["EXCESS_CALORIES"]\
            + self.feed_shutoff_kcals

        return (self.feed_shutoff_kcals,self.feed_shutoff_fat,self.feed_shutoff_protein)