...
for d in range(0,NDAYS):
	...
	if(d==0): #first Day
		...
		model += (used_area[d]==is_harvest[d]*built_area[d],
			"Is_Harvest_"+str(d)+"_Constraint")
		...
		model += (seaweed_wet_on_farm[d] == 
			...
			- is_harvest[d]*(seaweed_wet_on_farm[d-1]-used_area[d-1]*MINIMUM_DENSITY)*(HARVEST_LOSS/100),
