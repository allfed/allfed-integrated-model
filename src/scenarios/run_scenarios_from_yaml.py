...
def get_input_args(args):
    ...
    if len(args) < 3:
        ...
        print("\nError: fewer than 3 arguments supplied.")
        sys.exit(1)

    if len(args) > 3:
        print_usage_message(repo_root)
        print("\nError: more than 3 arguments supplied.")
        sys.exit(1)

    # there are 3 arguments
    if not os.path.exists(Path(repo_root) / "scenarios" / args[2]):
        print_usage_message(repo_root)
    ...
    try:
        ...
        print_usage_message(repo_root)
        print(
            "\nInvalid plotting arguments provided. Use True or False for the first two arguments."
        )
        sys.exit(1)

    yaml_filename = args[2]

    return show_country_figures, show_map_figures, yaml_filename
...
def main(args):
    ...
    """
    (show_country_figures, show_map_figures, yaml_filename) = get_input_args(args)

    config_data = load_config_data(yaml_filename)
    run_scenarios_from_yaml(config_data, show_country_figures, show_map_figures)
